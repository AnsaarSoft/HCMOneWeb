...
namespace HCM.UI.Pages.MasterDataSetup
{
    public partial class Station
    {
        ...
        private bool FilterFunc(MstStation element) => FilterFunc(element, searchString1);

        MstStation oModel = new MstStation();
        private IEnumerable<MstStation> oList = new List<MstStation>();
                ...
                if (!string.IsNullOrWhiteSpace(oModel.Code) && !string.IsNullOrWhiteSpace(oModel.Description))
                {
                    ...
                    else
                    {
                        if (oModel.Id == 0)
                        {
                            ...
                            res = await _mstStation.Insert(oModel);
                        }
                        else
                        {
                            oModel.UpdatedBy = LoginUser;
                            res = await _mstStation.Update(oModel);
                        }
                    }
                    if (res != null && res.Id == 1)
                    {
                        ...
                        Navigation.NavigateTo("/Station", forceLoad: true);
                    }
                    else
                    {
        ...
        private async Task GetAllStations()
        {
            try
            {
                oList = await _mstStation.GetAllData();
            }
            catch (Exception ex)
            {
                Logs.GenerateLogs(ex);
        ...
        private bool FilterFunc(MstStation element, string searchString1)
        {
        ...
        protected async override Task OnInitializedAsync()
        {
            try
            {
                ...
                if (Session != null)
                {
                    ...
                    if (res.Where(x => x.CMenuID == 21 && x.UserRights == true).ToList().Count > 0)
                    {
