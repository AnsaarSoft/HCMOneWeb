...
namespace HCM.UI.Data.MasterData
{
    public class MstGradingService : IMstGrading
    {
        ...
        private const string CacheKey = "GradingMaster";

        public MstGradingService(IMemoryCache memoryCache)
        {
        ...
        public async Task<List<MstGrading>> GetAllData()
        {
            try
            {
                if (_memoryCache.TryGetValue(CacheKey, out IEnumerable<MstGrading> oListCache))
                {
        ...
        public async Task<ApiResponseModel> Insert(List<MstGrading> oMstGrading)
        {
            ...
            try
            {
                var request = new RestRequest("MasterData/addGradingList", Method.Post);
                request.AddJsonBody(oMstGrading);
                var res = await _restClient.ExecuteAsync(request);
                if (res.IsSuccessful)
                {
        ...
        public async Task<ApiResponseModel> Update(List<MstGrading> oMstGrading)
        {
            ...
            try
            {
                var request = new RestRequest("MasterData/updateGradingList", Method.Post);
                request.AddJsonBody(oMstGrading);
                var res = await _restClient.ExecuteAsync(request);
                if (res.IsSuccessful)
                {
